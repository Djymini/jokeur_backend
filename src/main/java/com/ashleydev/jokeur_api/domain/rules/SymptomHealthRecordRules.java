...
public class SymptomHealthRecordRules {
    ...
    public static void checkNotAlreadyAdded(
            Long symptomId,
            Long healthRecordId,
            List<SymptomHealthRecordEntity> existingSymptoms
    ){
        boolean alreadyExists = existingSymptoms.stream().anyMatch(shr ->
                shr.getSymptom().getId().equals(symptomId));
