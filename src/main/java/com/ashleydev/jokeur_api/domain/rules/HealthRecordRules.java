...
public class HealthRecordRules {
  ...
  public HealthRecordRequestDTO normalize(HealthRecordRequestDTO dto) {
    return new HealthRecordRequestDTO(
      dto.ownerId(),
      trimToNull(dto.petName()),
      dto.animalType(),
      dto.breed(),
      dto.sex(),
      dto.birthDate(),
      dto.currentWeight(),
      dto.color(),
      trimToNull(dto.identificationNumber()),
      trimToNull(dto.tattooNumber()),
      trimToNull(dto.allergy())
    );
  }

  public HealthRecordUpdateDTO normalize(HealthRecordUpdateDTO dto) {
    if (dto.getPetName() != null) {
      dto.setPetName(trimToNull(dto.getPetName()));
    }
    if (dto.getIdentificationNumber() != null) {
      dto.setIdentificationNumber(trimToNull(dto.getIdentificationNumber()));
    }
    if (dto.getTattooNumber() != null) {
      dto.setTattooNumber(trimToNull(dto.getTattooNumber()));
    }
    if (dto.getAllergy() != null) {
      dto.setAllergy(trimToNull(dto.getAllergy()));
    }
    return dto;
  }
