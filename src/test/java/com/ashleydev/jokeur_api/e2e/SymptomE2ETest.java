...
@ActiveProfiles("e2e")
class SymptomE2ETest extends TestContainerConfig {
    ...
    void shouldGetAllSymptoms() throws Exception {
        ...
        symptom.setName("Diarrhée");
        symptomRepository.save(symptom);

        String response = mockMvc.perform(get("/symptoms")
                        .header("Authorization", "Bearer " + jwt))
                .andExpect(status().isOk())
                .andReturn()
                .getResponse()
        ...
        for (int i = 0; i < symptoms.length(); i++) {
            ...
            if(s.getString("name").equals("Diarrhée")) {
                found = true;
            }
